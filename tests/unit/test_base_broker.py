...
class MockBroker(BaseBroker):
    ...
    def place_order(self, symbol: str, quantity: int, order_type: str, price: Optional[float] = None):
        return {"symbol": symbol, "quantity": quantity, "order_type": order_type, "price": price}
    ...
    def get_trade_history(self, start_date: str, end_date: str):
        return [{"date": start_date, "symbol": "AAPL", "quantity": 10, "price": 150.00}]
...
def test_cannot_instantiate_base_broker():
    ...
    with pytest.raises(TypeError, match="Can't instantiate abstract class BaseBroker with abstract methods"):
        BaseBroker()
